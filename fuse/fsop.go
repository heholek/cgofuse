...
// FileSystemOpendirEx is the interface that wraps the OpendirEx method.
//
// OpendirEx is similar to Opendir except that it allows direct manipulation of
// the FileInfo_t struct (which is analogous to the FUSE struct fuse_file_info).
type FileSystemOpendirEx interface {
	OpendirEx(path string, fi *FileInfo_t) int
}
